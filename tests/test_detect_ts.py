...
class TestAnomalyDetection(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.data5 = pd.read_csv('test_data_5.csv', index_col='timestamp',
                       parse_dates=True, squeeze=True,
                       date_parser=self.dparserfunc)        
        self.a_series = AnomalySeries(self.data) 
        self.a_series_multithreaded = AnomalySeries(self.data, _get_dask_series(self.data))  
    ...
    def test_perform_threshold_filter_multithreaded(self):
        ...
        filtered_results = _perform_threshold_filter(AnomalyResultSeries(results['anoms']), periodic_max, 'med_max')
    ...
    def test_perform_threshold_filter_multithreaded(self):
        results = anomaly_detect_ts(self.data, max_anoms=0.02, direction='both',
                                      only_last=None, plot=False, multithreaded=True)
        periodic_max = self.data.resample('1D').max()
        filtered_results = _perform_threshold_filter(AnomalyResultSeries(results['anoms'], _get_dask_series(results['anoms'])), periodic_max, 'med_max', multithreaded=True)
        self.assertTrue(isinstance(filtered_results, pd.Series))
        self.assertEquals(4, len(filtered_results))
